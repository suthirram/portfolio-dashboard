...
function Card({label, inr, eur, positive, negative, highlight, accent, onClick}: CardProps) {
    ...
    return (
        <div className={cardClass(highlight)} style={{...cardStyle, ...(onClick ? {cursor: 'pointer'} : {})}}
             onClick={onClick}>
...
function InvestedBreakdownCard({stocksInr, stocksEur, goldInvestedInr, onClick}: {
    ...
    const rows: BreakdownRow[] = [
        ...
        ...(stocksEur != null && stocksEur > 0 ? [{
            name: 'EUR',
            symbol: '€',
            value: stocksEur,
            bg: 'var(--breakdown-eur-bg)'
        }] : []),
        ...(goldInvestedInr != null ? [{
            name: 'Gold',
            symbol: '₹',
            value: goldInvestedInr,
            bg: 'var(--breakdown-gold-bg)'
        }] : []),
    ]
    return (
...
function CurrentBreakdownCard({stocksInr, stocksEur, goldCurrentInr, onClick}: {
    ...
    if (stocksEur != null && stocksEur > 0) rows.push({
        name: 'EUR',
        symbol: '€',
        value: stocksEur,
        bg: 'var(--breakdown-eur-bg)'
    })
    if (goldCurrentInr != null) rows.push({
        name: 'Gold',
        symbol: '₹',
        value: goldCurrentInr,
        bg: 'var(--breakdown-gold-bg)'
    })
...
function FlipCard({onFlip, children}: { onFlip: () => void; children: React.ReactNode }) {
    ...
    return (
        <div className={cls}
             style={{flex: '1 1 174px', minWidth: 180, minHeight: 174, display: 'flex', flexDirection: 'column'}}
             onClick={handleClick}>
